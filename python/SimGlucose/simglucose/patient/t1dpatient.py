...
# References
# http://www.isbgroup.eu/Workshops2011/dallaman.pdf
# https://www.ncbi.nlm.nih.gov/pmc/articles/PMC2681269/pdf/dst-03-0044.pdf
# https://www.ncbi.nlm.nih.gov/pmc/articles/PMC4074748/pdf/dia.2013.0377.pdf
# https://www.ncbi.nlm.nih.gov/pmc/articles/PMC4454102/pdf/10.1177_1932296813514502.pdf
...
class T1DPatient(Patient):
    ...
    def step(self, action):
        ...
        # print("In patient", self._params.Vmx, self._params.kp3)
    ...
    def model(t, x, action, params, last_Qsto, last_foodtaken):
        ...
        # intestine
        dxdt[2] = kgut * x[1] - params.kabs * x[2]                      # Eqn 3

        # Rate of appearance
        Rat = params.f * params.kabs * x[2] / params.BW                 # Eqn 13
        # Glucose Production
        EGPt = params.kp1 - params.kp2 * x[3] - params.kp3 * x[8]       # Eqn 10
        # Glucose Utilization
        Uiit = params.Fsnc

        # renal excretion
        if x[3] > params.ke2:
            Et = params.ke1 * (x[3] - params.ke2)  # Eqn 27
        else:
            Et = 0
        ...
        dxdt[3] = max(EGPt, 0) + Rat - Uiit - Et - \
            params.k1 * x[3] + params.k2 * x[4]             # Eqn 1
        dxdt[3] = (x[3] >= 0) * dxdt[3]

        Vmt = params.Vm0 + params.Vmx * x[6]
        Kmt = params.Km0
        Uidt = Vmt * x[4] / (Kmt + x[4])        # Eqn 15  21
        dxdt[4] = -Uidt + params.k1 * x[3] - params.k2 * x[4]  # Eqn 1
        dxdt[4] = (x[4] >= 0) * dxdt[4]

        # insulin kinetics
        dxdt[5] = -(params.m2 + params.m4) * x[5] + params.m1 * x[9] + params.ka1 * \
            x[10] + params.ka2 * x[11]  # plus insulin IV injection u[3] if needed   # Eqn 3
        It = x[5] / params.Vi                                                        # Eqn 3
        dxdt[5] = (x[5] >= 0) * dxdt[5]

        # insulin action on glucose utilization
        dxdt[6] = -params.p2u * x[6] + params.p2u * (It - params.Ib)   # Eqn 18

        # insulin action on production
        dxdt[7] = -params.ki * (x[7] - It)                          # Eqn 11

        dxdt[8] = -params.ki * (x[8] - x[7])                        # Eqn 11

        # insulin in the liver (pmol/kg)
        dxdt[9] = -(params.m1 + params.m30) * x[9] + params.m2 * x[5]   # Eqn 3
        dxdt[9] = (x[9] >= 0) * dxdt[9]

        # subcutaneous insulin kinetics
        dxdt[10] = insulin - (params.ka1 + params.kd) * x[10]  # Eqn 24 # A16
        dxdt[10] = (x[10] >= 0) * dxdt[10]

        dxdt[11] = params.kd * x[10] - params.ka2 * x[11]               # A16
        dxdt[11] = (x[11] >= 0) * dxdt[11]

        # subcutaneous glcuose
        dxdt[12] = (-params.ksc * x[12] + params.ksc * x[3])        # Eqn 26
